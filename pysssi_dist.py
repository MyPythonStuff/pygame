...
class	gamestate:
	def	__init__(self):
		...
		self.invaders_speed = 5			# how many invader steps per frame?
		...
		self.meteor_field_enabled = 0	# is this a meteor field wave?
...
def	init_initwave():
	...
	STATE.wave_lo = (STATE.waveno - 1) & 1
	STATE.wave_hi = (STATE.waveno - 1) / 2
...
def	canvas_greet():
	...
	nn = 9		# cheesy line spacing onscreen
	...
	tmp = "Last: " + `STATE.score` + " --- High: " + `STATE.highscore`
...
def	aliens_think_invaders():
	...
	if STATE.you.dead:
		return
	for a in range( STATE.invaders_speed):
...
def	you_think():
	...
	if keybits[ K_LCTRL] or keybits[ K_RCTRL]:
		if (STATE.you.gunheat == 0) or (not (prevkeybits[ K_LCTRL] or prevkeybits[ K_RCTRL])):
...
##################################################################
def joystick_test():
	global joystick1, joyinitted, joydot
	if not joyinitted:
		joyinitted = 1
		try:
